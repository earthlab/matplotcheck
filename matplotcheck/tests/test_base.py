...
def pd_bar_plt():
    """Create a pandas dataframe for testing"""
    # Can a fixture take data from another fixture??
    pd_df = pd.DataFrame(np.random.randint(0, 100, size=(25, 2)), columns=list("AB"))
    fig, ax = plt.subplots()
...
def test_options(pd_line_plt):
    ...
    with pytest.raises(ValueError, match="Plot_type to test must be either: scatter, bar or line"):
