...
class TimeSeriesTester(PlotTester):
	...
	def assert_xticks_reformatted(self, tick_size='large', loc_exp=None, m='x ticks have not been reformatted properly'):
		...
		if not loc_exp:
			...
			if tick_size == 'large':
				...
				test_date = self.ax.xaxis.get_minor_formatter().format_data(735141).replace(' ', '').lower() #September 30, 2013
			else:
				raise ValueError("tick_size must be on of the following string ['large', 'small']")
			if loc_exp == 'decade' or loc_exp == 'year': accepted_responses = ['2013']
			elif loc_exp == 'month': accepted_responses = ['sep', 'september']
			elif loc_exp == 'week' or loc_exp == 'day': accepted_responses = ['sep30', 'september30']
			else:
				raise ValueError("loc_exp must be one of the following strings ['decade', 'year', 'month', 'week', 'day', None]")
			assert test_date in accepted_responses, m

	def assert_xticks_locs(self, tick_size='large', loc_exp=None, m='Incorrect X axis tick locations'):
		...
		if not loc_exp:
			...
			if loc_exp == 'decade': inc = relativedelta(years=10)
			elif loc_exp == 'year': inc = relativedelta(years=1)
			elif loc_exp == 'month': inc = relativedelta(months=1)
			elif loc_exp == 'week': inc = relativedelta(days=7)
			elif loc_exp == 'day': inc = relativedelta(days=1)
			else:
				raise ValueError("loc_exp must be one of the following strings ['decade', 'year', 'month', 'week', 'day'] or None")
	...
	def _my_range(self, start, end, step):
		"""helper function for assert_xticks_locs
			alternative to range to use in a for loop. my_range allows for dataype other than ints.
			both start and end are included in the loop.
	...
	def assert_xdata_date(self, x_exp, m='X-axis is not in appropriate date format'):
		...
		Matplotlib changes datetime to floats represeting number of days since day 0. 
		If you are using dates prior to year 270, this assertion will fail.
