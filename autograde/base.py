...
class PlotTester(object):
	...
	def assert_plot_type(self, plot_type=None):
		...
		if plot_type:
			if plot_type == 'scatter':
				assert self._is_scatter(), 'Plot is not of type {0}'.format(plot_type)
			elif plot_type == 'bar':
				assert self.ax.patches, 'Plot is not of type {0}'.format(plot_type)
			else:
				raise ValueError("plot_type must be on of the following string ['scatter', 'bar']")
	...
	def assert_title_contains(self, lst, title_type = 'either'):
		...
		parameters
		----------
		lst: list of strings to be searched for in title. strings must be lower case.
	...
	def assert_axis_off(self, m='Axis lines are displayed on plot'):
		...
		"""
		flag = False
		if self.ax.axison == False: flag = True
		elif self.ax.xaxis._visible == False and self.ax.yaxis._visible == False: flag = True
		elif self.ax.xaxis.get_gridlines() == [] and self.ax.yaxis.get_gridlines() == []: flag = True
		assert flag, m
	...
	def get_xy(self, points_only=False, xtime=False):
		...
		if xtime: xy_data['x'] = mdates.num2date(xy_data['x'])
		return xy_data

	def assert_xydata(self, xy_expected, xcol=None, ycol=None, points_only=False, xtime=False, xlabels=False, tolerence=0, m ='Incorrect data values'):
		...
		if type(xy_expected) == pd.DataFrame or type(xy_expected) == gpd.geodataframe.GeoDataFrame:
			...
			if tolerence > 0:
				if xtime: raise ValueError("tolerence must be 0 with datetime on x-axis")
				np.testing.assert_allclose(xy_data['x'], xy_expected[xcol], rtol = tolerence, err_msg = m)
				np.testing.assert_allclose(xy_data['y'], xy_expected[ycol], rtol = tolerence, err_msg = m)
			else:
