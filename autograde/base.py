...
class PlotTester(object):
    """Base class object to test Matplotlib plots

    Parameters
    ----------
    object: Matplotlib.Axes.axes object
        The matplotlib axes object to be tested
    """
    ...
    def _is_scatter(self):
        ...
        If plot contains scatter points as well as lines, functions will
        return true.
        ...
        if self.ax.collections:
            ...
            for l in ax.lines:
                if (l.get_linestyle() == 'None' or l.get_linewidth() == 'None' or l.get_linewidth() == 0):
                    return True
        return False
    ...
    def assert_plot_type(self, plot_type=None):
        ...
        if plot_type:
            if plot_type == "scatter":
                assert self._is_scatter(), "Plot is not of type {0}".\
                    format(plot_type)
            elif plot_type == "bar":
                assert self.ax.patches, "Plot is not of type {0}".\
                    format(plot_type)
            else:
                raise ValueError("""plot_type must be on of the following string
                                  ["scatter", "bar"]""")
    ...
    def get_titles(self):
        ...
        -------
        suptitle: str
            The figure title of the Figure ax is on. If none, returns empty
            string
        title: str
            Title on the axes. If none, this is an empty string.
        """
    ...
    def assert_title_contains(self, lst, title_type = "either"):
        """Searches a figure or axes title for a word of words contained in a
        string. Asserts title contains each string in lst. Whether we test the
        axes title or figure title is described in title_type.

        Parameters
        ----------
        lst: list of strings
            List of words (strings) that are searched for in the title. Strings
            should be lower case.
        title_type: str (value)
            One of the following ["figure", "axes", "either"]. Figure
            designates searching the matplotlib figure (subtitle) for a figure
            title. Axes, searches for an axes title. Use "either" if you want
            to search both the figure and the axis title for a string or
            strings in the list. The combined title will be tested.
        """
        ...
        if title_type == "either":
            ...
            raise ValueError("""title_type must be one of the following:
                             figure, axes, either.""")
        ...
        if not lst:
            ...
            for s in lst:
              assert s.lower().replace(" ", "") in title, """Title does not contain
              expected text:{0}""".format(s)
    ...
    def get_caption(self):
        ...
        Returns
        -------
        matplotlib.text.Text if text is found in bottom right. None if no text is
        ...
        for tex in self.ax.get_figure().texts:
            ...
            if (ax_position.ymin - .1 < tex_position[1] < ax_position.ymin) and (ax_position.xmax - .5 < tex_position[0] < ax_position.xmax):
                caption = tex
                break
        return caption
    ...
    def assert_caption_contains(self, strings_exp):
        ...
        strings_exp is a list of lists. Each internal list is a list of strings
        where at least one string must be in the caption, barring capitalization.
        Once a string is found, it is removed from the caption, therefore, order
        does matter. This is to enforce no overlap in found strings.
        ...
        for s in lst:
            if s.lower().replace(" ","") in caption:
                ...
                flag = True
            break
        assert flag, "Caption does not contain expected string: {0}".format(s)
    ...
    def assert_axis_off(self, m="Axis lines are displayed on plot"):
        ...
        if self.ax.axison == False:
            flag = True
        elif self.ax.xaxis._visible == False and self.ax.yaxis._visible == False:
            flag = True
        elif self.ax.xaxis.get_gridlines() == [] and self.ax.yaxis.get_gridlines() == []:
            flag = True
        assert flag, m

    def assert_axis_label_contains(self, axis="x", lst=[]):
        """Asserts axis label contains each of the strings in lst. The axis that is tested is described
        in axis.
        ...
        parameters
        ---------
        axis: one of the following ["x","y"] stated which axis label to be tested
        lst: list of strings to be searched for in axis label. strings must be lower case.
        """
        if axis == "x":
            ...
            raise ValueError("axis must be one of the following: x or y ")
    ...
    def assert_lims(self, lims_expected, axis='x'):
        ...
        paramters
        ---------
        lims_expected: list of length 2 containing expected min and max for x axis limits
        ...
        if axis == "x":
            ...
            raise ValueError("axis must be one of the following strings: x or y")
            assert np.array_equal(lims, lims_expected), "Incorrect limits on the {0} axis".format(axis)

    def assert_lims_range(self, lims_range, axis="x"):
        """Asserts limits along axis defined in variable axis are in range lims_range.
    ...
    def assert_equal_xlims_ylims(self, m="xlims and ylims are not equal"):
        ...
        parameters
        ---------
        m: string containing error message if assertion is not met
        """
    ...
    def assert_legend_titles(self, titles_exp):
        ...
        parameters
        ---------
        titles_exp: list of strings. Each string is expected be be in one subtitle. The number of strings is equal
        to the number of expected subtitles.
        """
        legends = self.get_legends()
        assert len(legends) == len(titles_exp), "Incorrect number of legend exist"

        titles = [leg.get_title().get_text().lower() for leg in legends]
        for title_exp in titles_exp:
        assert any(title_exp in s for s in titles), "Legend subtitle does not contain expected string: {0}".format(title_exp)
    ...
    def assert_legend_labels(self, labels_exp):
        ...
        parameters
        ---------
        labels_exp: list of lower case strings. Each string is an expected legend entry label.
        """
        legends = self.get_legends()
        assert legends, "Legend does not exist"

        legend_texts = [t.get_text().lower() for leg in legends for t in leg.get_texts()]
        assert len(legend_texts) == len(labels_exp), "Legend does not contain expected number of entries"
        assert set(legend_texts) == set(labels_exp), "Legend does not have expected labels"
    ...
    def assert_legend_accuracy_classified_image(self, im_expected, all_label_options):
        ...
        legend_dict = {}
        for p in [p for sublist in [leg.get_patches() for leg in legends] for p in sublist]:
        label = p.get_label().lower()
        legend_dict[p.get_facecolor()] = which_label(label, all_label_options)
        assert len([val for val in legend_dict.values() if val]) == len(all_label_options), "Incorrect legend labels"
        ...
        im_expected_labels = [[all_label_options[val][0] for val in row] for row in im_expected]
        assert np.array_equal(im_data_labels, im_expected_labels), "Incorrect legend to data relation"


    def assert_legend_no_overlay_content(self, m="Legend overlays plot contents"):
    ...
    def legends_overlap(self, b1, b2):
        ...
        """
        x_overlap = (b1[0][0]<=b2[1][0] and b1[0][0]>=b2[0][0]) or (b1[1][0]<=b2[1][0] and b1[1][0]>=b2[0][0])
        y_overlap = (b1[0][1]<=b2[1][1] and b1[0][1]>=b2[0][1]) or (b1[1][1]<=b2[1][1] and b1[1][1]>=b2[0][1])
    ...
    def get_xy(self, points_only=False, xtime=False):
        ...
        points_only: boolean
        xtime = boolean stating is the x axis contains datetime values
        ...
        """
        if points_only:
            xy_coords = [val for l in self.ax.lines if (l.get_linestyle() == 'None' or l.get_linewidth() == 'None') for val in l.get_xydata()] #.plot()
            xy_coords += [val for c in self.ax.collections if type(c) != matplotlib.collections.PolyCollection for val in c.get_offsets()] #.scatter()

        else:
            xy_coords = [val for l in self.ax.lines for val in l.get_xydata()] #.plot()
            xy_coords += [val for c in self.ax.collections for val in c.get_offsets()] #.scatter()
            xy_coords += [[(p.get_x()+(p.get_width()/2)), p.get_height()] for p in self.ax.patches] #.bar()
    ...
    def assert_xydata(self, xy_expected, xcol=None, ycol=None,
                      points_only=False, xtime=False, xlabels=False,
                      tolerence=0, m ='Incorrect data values'):
        ...
        ----------
        ax: Matplotlib Axes to be tested
        xy_expected: pandas dataframe or geopandas dataframe containing data expected to be on plot
        xcol: string title of column in xy_expected containing values along x_axis.
        If xy_expected contains this data in "geometry", set to None
        ycol: string title of column in xy_expected containing values along y_axis.
        If xy_expected contains this data in "geometry" set to None
        points_only: boolean, true if checking only points, false if checking all data on plot
        xtime: boolean expressing if the x-axis contains datetime values
        xlabels: boolean if using x axis labels rather than x data
        tolerence: measure of relative error allowed. For example, a value of .001 asserts values in array
        are within .001 of each other. ## this isn'"'t exactly correct.. ##
        m: string error message if assertion is not met
        """

        if type(xy_expected) == gpd.geodataframe.GeoDataFrame and not xcol:
            xy_expected = pd.DataFrame(data={'x':[p.x for p in xy_expected.geometry],
            'y':[p.y for p in xy_expected.geometry]}).dropna()
            xcol, ycol = 'x', 'y'
        if type(xy_expected) == pd.DataFrame or type(xy_expected) == gpd.geodataframe.GeoDataFrame:
            if xlabels:
            self.assert_xlabel_ydata(xy_expected, xcol=xcol, ycol=ycol)
            return
        xy_data = self.get_xy(points_only = points_only, xtime = xtime)
        xy_data, xy_expected = xy_data.sort_values(by='x'), xy_expected.sort_values(by=xcol)

        if tolerence > 0:
            if xtime:
                raise ValueError("tolerance must be 0 with datetime on x-axis")
                np.testing.assert_allclose(xy_data['x'], xy_expected[xcol],
                                           rtol = tolerence, err_msg = m)
                np.testing.assert_allclose(xy_data['y'], xy_expected[ycol],
                                           rtol = tolerence, err_msg = m)
            else:
                assert np.array_equal(xy_data['x'], xy_expected[xcol]), m
                assert np.array_equal(xy_data['y'], xy_expected[ycol]), m
        elif not xy_expected:
            pass
        else:
            raise ValueError('xy_expected must be of type: pandas dataframe or Geopandas Dataframe')
    ...
    def assert_xlabel_ydata(self, xy_expected, xcol, ycol, m='Incorrect Data'):
        ...
        m: string error message if assertion is not met
        """
        x_data = [''.join(c for c in l.get_text() if c.isdigit()) for l in self.ax.xaxis.get_majorticklabels()]
        y_data = self.get_xy()['y']
        xy_data = pd.DataFrame(data={'x': x_data, 'y': y_data})
        xy_expected, xy_data = xy_expected.sort_values(by=xcol), xy_data.sort_values(by='x')
        np.testing.assert_equal(np.array(xy_data['x']),np.array(xy_expected[xcol]), m)
        np.testing.assert_equal(np.array(xy_data['y']),np.array(xy_expected[ycol]), m)
    ...
    def get_slope_yintercept(self, path_verts):
        ...
        """
        slopes =[(path_verts[i+1,1]-path_verts[i,1])/(path_verts[i+1,0]-path_verts[i,0]) for i in range(len(path_verts)-1)]
        slope = sum(slopes)/len(slopes)
        return slope, path_verts[0,1]-(path_verts[0,0]*slope)

    def assert_line(self, slope_exp, intercept_exp, xtime = False, m='Expected line not displayed', m2= 'Line does not cover data set'):
        ...
        for l in self.ax.lines:
            ...
            if math.isclose(slope, slope_exp, abs_tol=1e-4) and math.isclose(y_intercept, intercept_exp, abs_tol=1e-4):
                flag_exist = True
    ...
    def assert_lines_of_type(self, line_types):
        ...
        if line_types:
            ...
            raise ValueError('each string in line_types must be from the following ["regression","onetoone"]')

        self.assert_line(slope_exp, intercept_exp, m='{0} line is not displayed properly'.format(line_type),
    ...
    def assert_num_bins(self, n=3, which_bins='positive'):
        ...
        'positive': all bins with values centered at a negative value
        """
        ...
        if which_bins == 'negative':
            ...
            raise ValueError("which_bins must be from list ['negative', 'positive']")
            assert n_bins >= n, 'Not enough {0} value bins on histogram'.format(which_bins)
