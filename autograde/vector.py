...
class VectorTester(PlotTester):
	...
	def _convert_length(self, arr, n):
		...
		if len(arr) == 1:
			...
			raise ValueError('Input array length is not of either expected values:1 or {0}'.format(n))
	...
	def get_points_by_attributes(self):
		...
		for c in (coll for coll in self.ax.collections if type(coll) == matplotlib.collections.PathCollection):
			colors, sizes = [tuple(color) for color in c.get_facecolors()], c.get_sizes()
			styles, offsets = [tuple(tuple(v) for v in p.vertices) for p in c.get_paths()], [tuple(o) for o in c.get_offsets()]
			n = len(offsets)
			colors, sizes, styles = self._convert_length(colors, n), self._convert_length(sizes, n), self._convert_length(styles, n)
			points_dataframe = points_dataframe.append(pd.DataFrame({'offset': offsets, 'color': colors,
				'msize': sizes, 'mstyle': styles}), ignore_index=True)
		
		points_grouped = [[data['offset'][i] for i in data.index] 
			for c, data in points_dataframe.groupby(['color', 'mstyle', 'msize'], sort=False)]
		return sorted([sorted(p) for p in points_grouped])

	def assert_points_grouped_by_type(self, data_exp, sort_column, m='Point attribtues not accurate by type'):
		...
		grouped_exp = [[(data.geometry[i].x, data.geometry[i].y) for i in data.index] 
	...
	def sort_collection_by_markersize(self):
		...
		for c in self.ax.collections:
			...
			x_data, y_data = [offset[0] for offset in offsets], [offset[1] for offset in offsets]
			if len(markersizes) == 1:
				markersize = [markersizes[0]] * len(offsets)
				df2 = pd.DataFrame({'x':x_data, 'y':y_data, 'markersize':markersize})
				df = df.append(df2)
			elif len(markersizes) == len(offsets):
				df2 = pd.DataFrame({'x':x_data, 'y':y_data, 'markersize':markersizes})
				df = df.append(df2)
	...
	def assert_collection_sorted_by_markersize(self, df_expected, sort_column):
		...
		np.testing.assert_almost_equal(np.array(df.x), np.array([p.x for p in df_expected.geometry]), decimal=6, err_msg='Markersize not based on {0} values'.format(sort_column))
		np.testing.assert_almost_equal(np.array(df.y), np.array([p.y for p in df_expected.geometry]), decimal=6, err_msg='Markersize not based on {0} values'.format(sort_column))
	...
	def _convert_multilines(self, df, column_title):
		...
		for i, row in dfout.iterrows():
			...
			if type(seg) == shapely.geometry.linestring.LineString:
				...
				raise ValueError('Segment is not of either expected type: MultiLinestring, LineString')
		return dfout
	...
	def get_lines(self):
		...
		"""
		lines = [[tuple(coords) for coords in seg] for c in self.ax.collections if type(c) == matplotlib.collections.LineCollection for seg in c.get_segments()]
	...
	def get_lines_by_collection(self):
		...
		"""
		lines_grouped = [[[tuple(coords) for coords in seg] for seg in c.get_segments()] for c in self.ax.collections if type(c) == matplotlib.collections.LineCollection]
	...
	def get_lines_by_attributes(self):
		...
		for c in (coll for coll in self.ax.collections if type(coll) == matplotlib.collections.LineCollection):
			segs = [[tuple(coords) for coords in s] for s in c.get_segments()] 
			colors, widths, styles = [tuple(color) for color in c.get_colors()], c.get_linewidth(), [self._convert_linestyle(ls) for ls in c.get_linestyle()]
			n = len(segs)
			colors, widths, styles = self._convert_length(colors, n), self._convert_length(widths, n), self._convert_length(styles, n)
			lines_dataframe = lines_dataframe.append(pd.DataFrame({'seg': segs, 'color': colors, 'lwidth': widths, 'lstyle': styles}), ignore_index=True)

		lines_grouped = [[data['seg'][i] for i in data.index] for c, data in lines_dataframe.groupby(['color', 'lwidth', 'lstyle'], sort=False)]
		return sorted([sorted(l) for l in lines_grouped])
	...
	def assert_lines(self, lines_expected, m='Incorrect Line Data'):
		...
		if type(lines_expected) == gpd.geodataframe.GeoDataFrame:
			lines_expected = lines_expected[lines_expected['geometry'].is_empty == False].reset_index(drop=True)
			fig, ax_exp = plt.subplots()
			lines_expected.plot(ax=ax_exp)
			lines_exp = VectorTester(ax=ax_exp).get_lines()
			plt.close(fig)
			np.testing.assert_equal(sorted(self.get_lines().lines), sorted(lines_exp.lines), m)
		elif not lines_expected:
			pass
		else:
			raise ValueError('lines_expected is not expected type: GeoDataFrame')

	def assert_lines_grouped_by_type(self, lines_expected, sort_column, m='Line attributes not accurate by type'):
		...
		if type(lines_expected) == gpd.geodataframe.GeoDataFrame:
			...
			for typ, data in lines_expected.groupby(sort_column):
				data.plot(ax=ax_exp)
			grouped_exp = [[[tuple(coords) for coords in seg] for seg in c.get_segments()] for c in ax_exp.collections if type(c) == matplotlib.collections.LineCollection]
	...
	def get_polygons(self):
		...
		"""
		output = [[tuple(coords) for coords in path.vertices] for c in self.ax.collections if type(c)==matplotlib.collections.PatchCollection for path in c.get_paths()]
		...
		if polygons_expected:
			...
			if dec:
				...
				for i in range(len(polygons)):
					np.testing.assert_almost_equal(polygons[i], polygons_expected[i], decimal=dec, err_msg=m)
			else: 
